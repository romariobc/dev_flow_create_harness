...
**Branch de domínio** — branch derivada de `main` (`dominio/<nome>`, ex.: `dominio/saude`) que adiciona ao trunk agnóstico os guardrails, vocabulário e exemplos específicos de um domínio. Não confundir com plataforma/visão (web, mobile), que é detalhe de projeto e fica em `context/`, nunca em uma branch — nem com `context/dominio.md` do projeto, que documenta o domínio de negócio de um projeto específico, não os guardrails que valem para todos os projetos daquele domínio. Ver `ARQUITETURA.md`, seção "Variação por domínio: branches".
